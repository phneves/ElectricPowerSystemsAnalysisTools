...
%fprintf('DEBUG> simulacao.m')

%pneves: function used to configure software, tolerance and so on
[software, tol] = ConfigInitialParameters;



%%
%pneves: time tracking 
tic; % iniciando contagem de tempo computacional

fprintf('\n> ---------------------------------\n')
fprintf('> Loadflow - v.4 -                 \n')
...
for k = 1:nb
    numext(k)   = barras(k,1);                  %pneves: bar number
    tipo(k)     = barras(k,2);                  %pneves: bar type
    v(k)        = barras(k,3);                  %pneves: V
    ang(k)      = barras(k,4) * graus_to_rad;   %pneves: angle*pi/180
    pg(k)       = barras(k,5)/baseMVA;          %pneves: 
...
while abs(maxDP) > tol | abs(maxDQ) > tol
    ...
    %pneves: iter/itmax shows the progress. Won't be used here
...
function [software, tol] = ConfigInitialParameters

    %pneves: keeping this for history purpose. Won't be used here.
    %Software utilizado (matlab/octave)
    software	=	'matlab';
    %software	=	'octave';

    %pneves: tolerance will be used to test convergence. 
    %tol		=	0.000000000001;
    tol         =	0.00001;
