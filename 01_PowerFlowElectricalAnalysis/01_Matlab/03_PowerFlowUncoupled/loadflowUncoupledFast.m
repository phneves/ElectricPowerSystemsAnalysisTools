...
while abs(maxDP) > tol | abs(maxDQ) > tol
    ...
    for k = 1:nb
        ...
        if tipo(k) ~= 3
            ...
            pcalc_pathUncoupled(iter+1,k) = pcalc(k);
            pcalc_timeUncoupled(iter+1,k)=toc;
            pcalc_kUncoupled(iter+1,k)=k;
            
            qcalc_pathUncoupled(iter+1,k) = qcalc(k);
            qcalc_timeUncoupled(iter+1,k) = toc;
            qcalc_kUncoupled(iter+1,k)=k;
